...
<xsl:text>    owns: </xsl:text><xsl:value-of
       select="count(/standards//standard/document[@orgid=$mykey])"/><xsl:text>&#x0A;</xsl:text>
<xsl:text>    references:&#x0A;</xsl:text>
<xsl:apply-templates select="/*//standard/document[@orgid=$mykey]" mode="liststandard"/>

<xsl:text>  coverdocs:&#x0A;</xsl:text>
<xsl:text>    owns: </xsl:text><xsl:value-of
       select="count(/standards//coverdoc/document[@orgid=$mykey])"/><xsl:text>&#x0A;</xsl:text>
<xsl:text>    references:&#x0A;</xsl:text>
<xsl:apply-templates select="/*//coverdoc/document[@orgid=$mykey]" mode="liststandard"/>
...
<!-- Create HTML Taxonomy Tree -->

<xsl:template match="taxonomy" mode="html-taxonomy">
  <xsl:result-document href="_includes/taxonomy.html" method="html">
    <div class="taxonomy">
      <ul>
        <xsl:apply-templates mode="html-taxonomy"/>
      </ul>
    </div>
  </xsl:result-document>
</xsl:template>
...
<xsl:template match="node" mode="html-taxonomy">
  ...
  <xsl:if test="./node">
    <ul>
      <xsl:apply-templates select="node" mode="html-taxonomy"/>
    </ul>
  </xsl:if>
  </li>
</xsl:template>
...
<xsl:template match="taxonomy" mode="json-taxonomy">
  <xsl:result-document href="_data/taxonomy.json">
    <div class="taxonomy">
      <ul>
        <xsl:apply-templates mode="json-taxonomy"/>
      </ul>
    </div>
  </xsl:result-document>
</xsl:template>
...
<xsl:template match="node" mode="json-taxonomy">
  <li>[<xsl:value-of select="@level"/><xsl:text>] </xsl:text>
  <a>
    <xsl:attribute name="href">
      <xsl:text>/node/</xsl:text>
...
-->
  <xsl:if test="./node">
    <ul>
      <xsl:apply-templates select="node" mode="json-taxonomy"/>
    </ul>
  </xsl:if>
  </li>
</xsl:template>
